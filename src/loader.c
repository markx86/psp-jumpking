  ...
  switch (current_job->status) {
    ...
    case LAZYJOB_CLOSE:
      if (current_job->file_size != (uint32_t)res) {
        loader_panic(
            "Read bytes mismatch: read %lu bytes out of %u", res,
            current_job->file_size);
      }
      if (qoi_start_job(
              &current_job->desc, current_job->read_dst,
...
void loader_lazy_swap_texture_ram(
    const char* path,
    void* dest,
    loader_lazyjob_finish_callback_t callback,
    void* callback_data) {
