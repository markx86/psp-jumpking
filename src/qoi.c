...
int
qoi_init_job(qoi_job_descriptor_t* desc, void* src, void* dst) {
  qoi_header_t* hdr;

  hdr = (qoi_header_t*)src;
...
int
qoi_lazy_decode(qoi_job_descriptor_t* d) {
  qoi_color_t* px;
  uint8_t b, run;
  int8_t dg;
  int is_rgba;
...
qoi_decode(
  ...
  width = bswap32(hdr->width);
  if (out_width != NULL)
    *out_width = width;

  height = bswap32(hdr->height);
  if (out_height != NULL)
    *out_height = height;
  ...
  px = (qoi_color_t){.r = 0, .g = 0, .b = 0, .a = 255};
