...
static void start(void) {
    ...
    king.sx[0] = 0; king.sx[1] = 0;
    king.sy[0] = 0; king.sy[1] = 0;
    framebuffer_index = 0;
    frame_counter = 0;
...
static void render(void) {
    ...
    if (frame_counter < 2) {
        ...
        if (scroll.target != scroll.current) {
            ...
            scroll.current = scroll.current + (short) ceilf(((float) (scroll.target - scroll.current)) * SCREEN_SCROLL_SPEED);
            // Scroll the screen.
            set_background_scroll(scroll.current);
            // Workaround to clear scrolling artifacts.
            level_force_clean_artifact_at(king.prev_sx[framebuffer_index], king.prev_sy[framebuffer_index], KING_SPRITE_WIDTH, KING_SPRITE_HEIGHT);
        }

        // Render new screen lines.
        if (scroll.current < scroll.min) {
            // If we're scrolling upwards, render the lines at the top of the screen.
            level_render_screen_lines_top(scroll.current, scroll.min - scroll.current);
            ...
            level_render_screen_lines_bottom(scroll.current, scroll.current - scroll.max);
        ...
        foreground_vertices = level_render_screen_section(king.sx[framebuffer_index] - 2, king.sy[framebuffer_index] - 2, KING_SPRITE_WIDTH + 4, KING_SPRITE_HEIGHT + 4, scroll.current);
    }
    ...
    // Render the player.
    king_render(&king.sx[framebuffer_index], &king.sy[framebuffer_index], scroll.current);
