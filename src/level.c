...
static __attribute__((section(".bss"), aligned(16))) char images_pool[3][LEVEL_SCREEN_BYTES];

static void screen_image_loaded_callback(void *data, uint32_t width, uint32_t height) {
    screen_handle_t *handle = (screen_handle_t *) data;
    handle->has_foreground = height > LEVEL_SCREEN_HEIGHT;
}

static void load_screen_image(screen_handle_t *handle, screen_image_loading_type_t loadType) {
    ...
    switch (loadType) {
        case LOAD_LAZY:
            loader_lazy_swap_texture_ram(file, handle->image, &screen_image_loaded_callback, handle);
            break;
        case LOAD_NOW:
    ...
    if (index < level.total_screens) {
        ...
        if (screen != last_screen_returned) {
            ...
            if (index == screen_handle_next.index) {
                ...
                screen_handle_previous.has_foreground = screen_handle_current.has_foreground;
                screen_handle_current.has_foreground = screen_handle_next.has_foreground;
                ...
                // - shift each handle's has_foreground flag
                screen_handle_next.has_foreground = screen_handle_current.has_foreground;
                screen_handle_current.has_foreground = screen_handle_previous.has_foreground;
...
static inline __attribute__((always_inline)) void render_screen(vertex_t* vertices, uint32_t image_offset, int tcc) {
    // Bind the foreground texture.
    sceGuTexMode(GU_PSM_8888, 0, 0, GU_TRUE);
    sceGuTexImage(0, LEVEL_SCREEN_IMAGEW, LEVEL_SCREEN_IMAGEH, LEVEL_SCREEN_IMAGEW, screen_handle_current.image + image_offset);
...
void level_render_foreground_on_top(vertex_t *vertices) {
    ...
    // Call render stub
    render_screen(vertices, LEVEL_SCREEN_IMAGEW * LEVEL_SCREEN_HEIGHT * 4, GU_TCC_RGBA);
    // Disable blending.
...
vertex_t *level_render_screen_section(short x, short y, short width, short height, uint32_t current_scroll) {
    // Clamp x coordinate within the screen bounds.
    if (x < 0) {
        x = 0;
    } else if (x + width >= LEVEL_SCREEN_WIDTH) {
